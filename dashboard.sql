...
SELECT
  ...
  CASE WHEN v.visitors_count > 0 THEN c.total_cost / v.visitors_count ELSE NULL END AS cpu,
  CASE WHEN l.leads_count > 0 THEN c.total_cost / l.leads_count ELSE NULL END AS cpl,
  CASE WHEN l.purchases_count > 0 THEN c.total_cost / l.purchases_count ELSE NULL END AS cppu,
  CASE WHEN c.total_cost > 0 THEN (l.revenue - c.total_cost) / c.total_cost * 100 ELSE NULL END AS roi
FROM visits_data v
FULL OUTER JOIN costs_data c ON
  v.visit_date = c.visit_date AND
  v.utm_source = c.utm_source AND
  v.utm_medium = c.utm_medium AND
  v.utm_campaign = c.utm_campaign
FULL OUTER JOIN leads_data l ON
